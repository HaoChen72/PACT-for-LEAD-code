function [outIm1,outIm2] = FrangiFilter2D(I, options)
...
%       .FrangiBetaOne : Frangi correction constant, default 0.5      'FrangiBetaOne', 调整线状物，块状物区别； 'FrangiBetaTwo'整体平滑参数   
...
beta  = 2*options.FrangiBetaOne^2;%调整线状与块状物的区别
c     = 2*options.FrangiBetaTwo^2;%控制线状物整体平滑的参数
% Make matrices to store all filterd images
ALLfiltered=zeros([size(I) length(sigmas)]);
ALLangles=zeros([size(I) length(sigmas)]);
% Frangi filter for all sigmas
%%当尺度因子 σ 与血管的实际宽度最匹配时，滤波器的输出最大。
%%所以作为空间尺度因子，迭代可以得到不同尺度的输出。局部特性分析的窗口矩形的半宽一般为 3σ
...
% output pixel value
if length(sigmas) > 1
    [outIm1,whatScale] = max(ALLfiltered,[],3);
    outIm2 = mean(ALLfiltered,3);
    outIm1 = reshape(outIm1,size(I));
    outIm2 = reshape(outIm2,size(I));
%     if(nargout>1)
...
%     end
    ...
    outIm1 = reshape(ALLfiltered,size(I));
    outIm2 = reshape(ALLfiltered,size(I));
%     if(nargout>1)
